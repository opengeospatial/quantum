...
* Fuel Efficiency: Minimizing fuel consumption for manoeuvres.
* Redundancy and Resilience: Ensuring the constellation can withstand the failure of one or more satellites.

The example described in section has a combination of coverage optimization and redundancy and resilience, however, the parameters could be changed and weighted to favor one element over another if _real_ data were being used.
